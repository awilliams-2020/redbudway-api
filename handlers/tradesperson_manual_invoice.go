...
func PostTradespersonTradespersonIDBillingManualInvoiceInvoiceIDVoidHandler(params operations.PostTradespersonTradespersonIDBillingManualInvoiceInvoiceIDVoidParams, principal interface{}) middleware.Responder {
	...
	stmt, err := db.Prepare("SELECT tp.name, tp.email, tp.number FROM tradesperson_manual_invoices i INNER JOIN tradesperson_profile tp ON i.tradespersonId=tp.tradespersonId WHERE i.tradespersonId=? AND i.invoiceId=?")
	if err != nil {
		return response
	}
	defer stmt.Close()
